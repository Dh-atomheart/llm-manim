...
ManimCE 0.20.x uses `Surface` or `axes.plot_surface(...)` for 3D surfaces. Do not use old ManimGL-style `ParametricSurface`; it is not exported by ManimCE.
...
Arrow3D does not support `tip_length`. Use `height` and `base_radius` to adjust the 3D arrow head:

```python
arrow = Arrow3D(ORIGIN, [2, 1, 2], height=0.3, base_radius=0.08, color=RED)
```
